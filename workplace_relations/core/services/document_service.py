...
class DocumentService:
    ...
    def handle_duplicates(self, documents: List[Document]) -> Dict[str, Any]:
        ...
        for identifier, docs in duplicates.items():
            ...
            for doc in sorted_docs[1:]:
                doc.metadata['duplicate_of'] = keeper.identifier
                doc.metadata['duplicate_action'] = 'marked'
                self.storage_service.delete_document(doc.file_path)
                actions.append(f"Deleted duplicate {doc.identifier} (keeping {keeper.identifier})")
    ...
    def process_document(self, document: Document) -> Optional[Document]:
        ...
        """
        try:
            if not document.file_path or not self.storage_service.document_exists(document.file_path):
                logger.warning(f"Document file not found: {document.file_path}")
                return None
            
            # Read original content
            original_content = self.storage_service.retrieve_document(document.file_path)
            if original_content is None:
                return None
            
            # Process content based on file type
            processed_content = self._process_content(original_content, document.file_type)
    ...
    def _process_content(self, content: bytes, file_type: Optional[str]) -> Optional[bytes]:
    ...
    def _create_processed_document(self, original_doc: Document, processed_content: bytes) -> Document:
    ...
    def _store_processed_document(self, document: Document, content: bytes) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to store processed document {document.file_path}: {e}")
            return False
    ...
    def get_document_statistics(self, documents: List[Document]) -> Dict[str, Any]:
        ...
        for doc in documents:
            ...
            if doc.published_date:
                try:
                    date_obj = datetime.strptime(doc.published_date, settings.DATE_FORMAT_DISPLAY)
