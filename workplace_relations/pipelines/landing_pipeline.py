...
class LandingPipeline(BasePipeline):
    ...
    def _store_in_mongodb(self, document: Document):
        ...
        try:
            ...
            self.logger.info(f"Stored document {document.identifier} (ID: {result.inserted_id})")
        except Exception as e:
            self.logger.error(f"Failed to store document {document.identifier} in MongoDB: {e}")
            raise
