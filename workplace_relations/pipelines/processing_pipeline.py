...
from workplace_relations.core.services import DocumentService
...
class ProcessingPipeline(BasePipeline):
    ...
    def _store_processed_document(self, document: Document):
        ...
        try:
            ...
            self.logger.info(f"Stored processed document {document.identifier} (ID: {result.inserted_id})")
        except Exception as e:
            self.logger.error(f"Failed to store processed document {document.identifier} in MongoDB: {e}")
            raise
